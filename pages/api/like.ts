    ...
    try {
        if(req.method === 'PUT'){ //<== Verifica se o método da requisição é "PUT"
            ...
            if(indexCurtidas != -1){ //<== Caso o valor da const indexCurtidas for diferente de -1 (0,1,2,...), significa que o usuário esta na lista de curtidas e portanto o remove usando splice.
                ...
                return res.status(200).json({ OK : 'Publicação descurtida com sucesso!' }); //<== Retorna uma mensagem de erro com o código 200 (OK), informando sucesso ao remover a curtida
            } else {
                publicacao.curtidas.push(usuario._id);
                await publicacaoModel.findByIdAndUpdate({_id : publicacao._id}, publicacao);
                return res.status(200).json({ OK : 'Publicação curtida com sucesso!' });
            }
        }
...
export default validarTokenJWT(conectarMongoDB(curtidaEndpoint));